...
export default function DiffViewerDrawer({ open, onClose, device, fromBackup, toBackup }: Props) {
  ...
  const copyDiff = async () => {
    ...
    try {
      ...
      message.success('Diff panoya kopyalandı')
    } catch {
    ...
    <Drawer
      title={
        <Space size={8} wrap>
          ...
          {hasChanges && (
            <>
              <Tag color="green">+{added} satır</Tag>
              <Tag color="red">−{removed} satır</Tag>
            </>
          )}
          {!loading && !hasChanges && diffQ.data && <Tag>Değişiklik yok</Tag>}
        </Space>
      }
      open={open}
      onClose={onClose}
    ...
    >
      {errored && (
        <Alert
          ...
          message="Diff alınamadı"
          description={(errored as any)?.message || 'Backend yanıt vermedi.'}
        />
      )}
          ...
          hasChanges ? (
            <div style={{ fontSize: 12 }}>
              <ReactDiffViewer
                ...
                useDarkTheme
                leftTitle={fromBackup ? `#${fromBackup.id} · ${dayjs(fromBackup.created_at).format('YYYY-MM-DD HH:mm')}` : 'önceki'}
                rightTitle={toBackup ? `#${toBackup.id} · ${dayjs(toBackup.created_at).format('YYYY-MM-DD HH:mm')}` : 'sonraki'}
              />
            </div>
          ) : (
            <Alert
              type="success" showIcon
              message="İki backup arası değişiklik yok"
              description={`Seçili iki konfigürasyon hash'leri farklı olabilir ama satır bazlı diff boş.`}
            />
          )
        )}
      </Spin>
