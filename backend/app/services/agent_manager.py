...
Backend sends SSH command requests; agents execute them and return results.
"""
import asyncio
import json
import time
...
def _is_command_allowed(command: str, mode: str, allowed_commands: list[str]) -> tuple[bool, str]:
    ...
    if mode == "whitelist":
        if not allowed_commands:
            # Empty whitelist = deny all non-safe commands
...
class AgentManager:
    ...
    async def disconnect(self, agent_id: str):
        ...
        # Fail any pending futures for this agent
    ...
    def set_security_config(self, agent_id: str, command_mode: str, allowed_commands_raw: Optional[str]):
        """Cache agent security config (called on connect and on config update)."""
    ...
    async def handle_message(self, agent_id: str, raw: str):
        ...
        if msg_type in ("ssh_result",):
            ...
            # Agent reports a command it blocked locally
            log.warning(
    ...
    async def execute_ssh_command(self, agent_id: str, device, command: str) -> dict:
        ...
        payload = {
            ...
            "command": command,
            # Send security config to agent for local enforcement too
    ...
    async def execute_ssh_config(self, agent_id: str, device, commands: list[str]) -> dict:
        ...
        # Validate each command in the config set
        sec = self.get_security_config(agent_id)
    ...
    async def send_key_rotate(self, agent_id: str, new_key: str) -> bool:
        """Send key rotation command to connected agent."""
        ws = self._connections.get(agent_id)
        if not ws:
            return False
        try:
    ...
    async def send_security_config(self, agent_id: str, command_mode: str, allowed_commands: list[str]) -> bool:
        """Push updated security config to a connected agent."""
        ws = self._connections.get(agent_id)
        if not ws:
            return False
        try:
    ...
    def _log_command_async(self, **kwargs):
        """Fire-and-forget: persist command log entry to DB."""
        try:
            loop = asyncio.get_running_loop()
    ...
    def _update_latency(self, agent_id: str, device_id: int, latency_ms: float, success: bool = True):
        """Update EMA latency in-memory and fire-and-forget DB persist."""
    ...
    async def load_latencies_from_db(self):
        """Populate in-memory latency cache from DB on startup."""
        try:
