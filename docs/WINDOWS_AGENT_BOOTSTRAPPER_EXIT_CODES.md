...
| 9 | `ExitInvalidInstallPath` | Install path parse / validation failed | Not currently emitted at the BuildPlan layer (paths are validated at Parse and surface as `ExitInvalidArguments`). Reserved for the future runtime path-mutation phase. |
...
Codes 7 / 8 / 9 / 10 are reserved in the constant table so the
numbers don't shift when PR-C / PR-F start emitting them. PR-B
emits 0 / 2 / 3 / 4 / 5 / 6 / 20 only.
